...
from modeling_complex_roberta import ComplexRobertaModel, ComplexRobertaLMHead, ComplexLinear, ComplexDropout
from modeling_complex_bert import ComplexBertPreTrainedModel, ComplexCrossEntropyLoss
from utils.semevalUtils import get_all_features_from_task_2
...
class RobertaForMultipleChoice(BertPreTrainedModel):
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
	            position_ids=None, head_mask=None, complex_ids = None):
		num_choices = input_ids.shape[1]
...
class RobertaForMultipleChoiceWithLM(BertPreTrainedModel):
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
	            position_ids=None, head_mask=None, complex_input_position_ids=None):
		...
		flat_position_ids = position_ids.view(-1, position_ids.size(-1)) if position_ids is not None else None
		flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1)) if token_type_ids is not None else None
		flat_complex_position_ids = complex_input_position_ids.view(-1, complex_input_position_ids.size(-1)) if complex_input_position_ids is not None else None
		if attention_mask.dim() == 3:
			flat_attention_mask = attention_mask.view(-1,
			                                          attention_mask.size(-1)) if attention_mask is not None else None
		else:
		...
		outputs = self.roberta(flat_input_ids, position_ids=flat_complex_position_ids, token_type_ids=flat_token_type_ids,
		                       attention_mask=flat_attention_mask, head_mask=head_mask)
...
class ComplexRobertaForMultipleChoiceWithLM(ComplexBertPreTrainedModel):
	r"""
	Inputs:
		**input_ids**: ``torch.LongTensor`` of shape ``(batch_size, num_choices, sequence_length)``:
			Indices of input sequence tokens in the vocabulary.
	...
	def __init__(self, config):
		super(ComplexRobertaForMultipleChoiceWithLM, self).__init__(config)
		...
		self.lamda1 = nn.Parameter(torch.rand(1) * 2 + 1)
		self.lamda2 = nn.Parameter(torch.rand(1) * 2 + 1)
		self.classifier = ComplexLinear(config.hidden_size, 1)
		self.init_weights()
		self.init_real_weight()
	
	def init_real_weight(self):
		# self.classifier
		from modeling_complex_bert import imag_norm_init
		imag_norm_init(self.classifier.weight)
		return
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
		...
		flat_position_ids = position_ids.view(-1, position_ids.size(-1)) if position_ids is not None else None
		flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1)) if token_type_ids is not None else None
		flat_complex_position_ids = complex_input_position_ids.view(-1, complex_input_position_ids.size(
			-1)) if complex_input_position_ids is not None else None
		...
		if attention_mask.dim() == 3:
			...
			flat_attention_mask = attention_mask.view(
				(-1,) + attention_mask.shape[-2:]) if attention_mask is not None else None
		# remember to make a break to monitor outputs[1] rather than outputs[0]
		outputs = self.roberta(flat_input_ids, position_ids=flat_complex_position_ids,
		                       token_type_ids=flat_token_type_ids,
		                       attention_mask=flat_attention_mask, head_mask=head_mask)
		
		sequence_output = outputs[0]
		complex_prediction_scores = self.lm_head(sequence_output) # here to make a breakpoint
		prediction_scores = complex_prediction_scores.abs()
		# from now outputs[1] backward works
		pooled_output = outputs[1].clone() #outputs1 error break here
		# from now outputs[1] backward doesn't work
		dropout_pooled_output = self.dropout(pooled_output) # here to make break
		complex_logits = self.classifier(dropout_pooled_output)
		complex_reshaped_logits = complex_logits.view(-1, num_choices)
		reshaped_logits = complex_reshaped_logits.abs()
		
		if True:
			# naive_ce = CrossEntropyLoss(ignore_index=-1, reduction='none')
			loss_fct = CrossEntropyLoss(ignore_index=-1, reduction='none')
			'''
			masked_lm_loss 是一个长度为 (batch_size * num_choices * max_seq_length,) 的 Tensor
			需要将其转换为 (batch_size, num_choices, max_seq_length)，再对每一个 (max_seq_length) 求平均
		...
		outputs = (reshaped_logits,
		           pooled_output.view(input_ids.shape[0], num_choices, -1).abs(),) + outputs[2:] # add hidden states and attention if they are here
		# outputs = (reshaped_logits / (2.0 * self.lamda1 * self.lamda1) + \
		#            -lm_loss / (2.0 * self.lamda2 * self.lamda2),
		#            pooled_output.view(input_ids.shape[0], num_choices, -1),) + outputs[
		#                                                                        2:]  # add hidden states and attention if they are here
		...
		if labels is not None:
			...
			# loss = loss + self.lamda1 * lm_loss_for_classification
			return_outputs = (loss,) + outputs
		
		return return_outputs  # (loss), reshaped_logits, (hidden_states), (attentions)
...
class GNNED(nn.Module):
	def __init__(self):
		super(GNNED, self).__init__()
		self.encode = GNNEncoder()
		self.decode = GNNDecoder()
	
	def forward(self, data, mid):
		# x, edge_index, edge_weight = data.x, data.edge_index, data.edge_attr
		x, edge_index, edge_weight = data.x, data.edge_index, None  # for GAT
		encode_output = self.encode(x)
		loss_encode = CrossEntropyLoss(encode_output, mid)
		decode_output = self.decode(encode_output)
		loss_decode = CrossEntropyLoss(decode_output, x)
		
		all_loss = loss_encode + loss_decode
...
class Complex_model(nn.Module):
	def __init__(self, args):
		...
		roberta_config.hidden_act = "complex_gelu"
		if args.get('with_lm'):
			self.roberta = ComplexRobertaForMultipleChoiceWithLM.from_pretrained(
				'pre_weights/roberta-large_model.bin', config=roberta_config)
		else:
			self.roberta = RobertaForMultipleChoice.from_pretrained(
				'pre_weights/roberta-large_model.bin', config=roberta_config)
	...
	def forward(self, x, labels=None):
		...
		complex_position_ids = torch.stack([j[5] for i in semantic_features for j in i], dim=0).reshape(
			(-1, num_choices,) + semantic_features[0][0][5].shape).to(
			self.args['device'])
		
		def convert2complexids(position_ids, complex_position_ids):
			tensor = torch.complex(position_ids.double(), complex_position_ids.double())
			return tensor
		...
		roberta_outputs = self.roberta(input_ids,
		                               ...
		                               position_ids=position_ids,
		                               labels=labels,
		                               complex_input_position_ids=complex_input_position_ids)
