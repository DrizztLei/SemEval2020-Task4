...
from model_modify import (
	...
	create_complex_datasets_with_kbert,
	train_and_finetune_complex,
	test)
...
from utils.MyDataset import MyDataLoader, MyComplexDataLoader
...
def build_parse():
	...
	parser.add_argument('--with-complex', action='store_true', default=False,
	                    help='Add complex space mechanism')
...
if __name__ == '__main__':
	...
	if default_args['with_complex']:
		# model = Complex_model(default_args)
		model = SOTA_goal_model(default_args)
	elif default_args['with_kegat']:
	...
	def realod_model_parameter(model, parameters):
	...
	if default_args['subtask_id'] == 'A':
		train_features = get_all_complex_features_from_task_1(
			'SemEval2020-Task4-Commonsense-Validation-and-Explanation-master/Training Data/subtaskA_data_mini.csv', # _mini for debug _all for overall performance
			'SemEval2020-Task4-Commonsense-Validation-and-Explanation-master/Training Data/subtaskA_answers_mini.csv',
		...
		test_features = get_all_complex_features_from_task_1(
	...
	train_dataset = create_complex_datasets_with_kbert(train_features, shuffle=True)
	dev_dataset = create_complex_datasets_with_kbert(dev_features, shuffle=False)
	# test_dataset = create_complex_datasets_with_kbert(test_features, shuffle=False)
	test_dataset = None
	...
	# sampler=test_sampler)
		...
		train_data = MyComplexDataLoader(train_dataset, batch_size=default_args['batch_size'])
		dev_data = MyComplexDataLoader(dev_dataset, batch_size=default_args['test_batch_size'])
		# test_data = MyComplexDataLoader(test_dataset, batch_size=default_args['test_batch_size'])
	...
	dev_acc, (train_pred_opt, dev_pred_opt) = train_and_finetune_complex(model, train_data, dev_data, default_args)
