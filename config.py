...
args = {
	...
	'ensemble_epochs': 32,
	'k_fold': 5,
	'use_cuda': True, # change here to switch cuda: torch.cuda.is_available(),
	'device': torch.device('cuda' if torch.cuda.is_available() else 'cpu'),
	'use_multi_gpu': False,
	'data_loader_num_workers': 4,
	'split_rate': 0.8,
	# 'max_seq_length': 128,  # 实际测试过 task2 最长 52，commonsenseQA 最长 80
	'max_seq_length': 256,  # 实际测试过 task2 最长 52，commonsenseQA 最长 80
	'exec_time': time.strftime("%Y.%m.%d-%H.%M.%S", time.localtime()),  # 执行程序的时间，主要用来保存最优 model
	'solo': False,
	'is_save_checkpoints': False,  # 是否保存权重信息
	# 'checkpoints_dir': '/data/chao_lei/ECNU/checkpoints',
	'checkpoints_dir': './checkpoints',
	'is_save_logs': True,  # 是否保存 tensorboard logs 信息
	'logs_dir': './logs/',
	'subtask_id': 'A',  # 执行哪个子任务 ['A', 'B']
