...
from models import BertForSequenceClassification, RobertaForMultipleChoice
from utils.MyDataset import MyDataLoader, MyDataset, MyComplexDataset
...
# %%
def train(model, train_data, optimizer, args, retain_graph=False, writer=None, epoch=0):
    model.train()
    ...
    is_gnn = 'SOTA_goal_model' in str(type(model)) or 'Complex' in str(type(model))
    # required_gradient_parameters = optimizer.param_groups[0]['params']
    ...
    for step, (x, y) in enumerate(pbar):
        ...
        if args['solo']:
            ...
            if not is_gnn:
                ...
                def convert2complexids(position_ids, complex_position_ids):
                    tensor = torch.complex(position_ids, complex_position_ids)
                    return tensor
                ...
                complex_position_ids = torch.stack([j[5] for i in x for j in i], dim=0).reshape(
                    (-1, num_choices,) + x[0][0][5].shape).to(
                    args['device'])

                complex_input_position_ids = convert2complexids(position_ids, complex_position_ids)

                # shape: [batch_size, choices_num, 3, max_seq_length]
                output = model(input_ids=input_ids,
                               attention_mask=attention_mask,
                               position_ids=complex_input_position_ids,
        ...
        if writer is not None and step % 10 == 0 and len(output) >= 5:
            roberta_loss = output[2]
            gcn_loss = output[3]
            fct_loss = output[4]

            writer.add_scalar('Loss/roberta', roberta_loss, step)
            writer.add_scalar('Loss/gcn_loss', gcn_loss, step)
            writer.add_scalar('Loss/fct_loss', fct_loss, step)

            if args["with_scl"]:
                contrastive_loss = output[5]
                writer.add_scalar('Loss/contrastive', contrastive_loss, step)

        loss.backward() #loss.backward(retain_graph=True)
        # nn.utils.clip_grad_norm_(required_gradient_parameters, max_norm=2, norm_type=2)
        # nn.utils.clip_grad_value_(required_gradient_parameters, clip_value=0.5)
        optimizer.step()

        pred = output[1].softmax(dim=1).argmax(dim=1, keepdim=True)
        pred_list.append(output[1].softmax(dim=1))

        softmax_result = output[1].softmax(dim=1)
        np_logits_list.append(softmax_result[::, 1].detach().cpu().numpy())
        np_labels_list.append(y.detach().cpu().numpy())
        np_preds_list.append(pred.detach().cpu().numpy().squeeze())
        # print(output[1].softmax(dim=1))

        """
        np_softmax_result = softmax_result.detach().cpu().numpy()
        np_preds = pred.detach().cpu().numpy().squeeze()
        np_label = y.detach().cpu().numpy()
        """

        correct += pred.eq(y.view_as(pred)).sum().item()
        count += len(y)
        del x, y, softmax_result, pred
    ...
    all_logits = np.concatenate(np_logits_list)
    all_preds = np.concatenate(np_preds_list)
    all_labels = np.concatenate(np_labels_list)
    weight = None
    eval_dict = eval_overall_performance(all_logits, all_preds, all_labels)

    if writer is not None:
        writer.add_scalar('train/Metrics/ACC', eval_dict['acc'], epoch)
        writer.add_scalar('train/Metrics/AUROC', eval_dict['auroc'], epoch)
        writer.add_scalar('train/Metrics/T_AP', eval_dict['t_ap'], epoch)
        writer.add_scalar('train/Metrics/T_F1', eval_dict['t_f1'], epoch)
        writer.add_scalar('train/Metrics/T_P', eval_dict['t_p'], epoch)
        writer.add_scalar('train/Metrics/T_R', eval_dict['t_r'], epoch)
        writer.add_scalar('train/Metrics/F_AP', eval_dict['f_ap'], epoch)
        writer.add_scalar('train/Metrics/F_F1', eval_dict['f_f1'], epoch)
        writer.add_scalar('train/Metrics/F_P', eval_dict['f_p'], epoch)
        writer.add_scalar('train/Metrics/F_R', eval_dict['f_r'], epoch)
        writer.add_scalar('train/Metrics/ECE', eval_dict['ece'], epoch)


    return train_loss / count, correct * 1.0 / count, torch.cat(pred_list, dim=0)


def test_with_dict(model, test_data, args, writer=None, epoch=0):
    model.eval()
    test_loss = 0
    correct = 0
    count = 0
    ...
    np_preds_list = []
    with torch.no_grad():
        for step, (x, y) in enumerate(tqdm(test_data)):
            # x, y = x.to(args['device']), y.to(args['device'])
            y = y.to(args['device'], non_blocking=True)
            if args['solo']:
                # shape: [batch_size, 3, max_seq_length]
            ...
            np_logits_list.append(softmax_result[::, 1].detach().cpu().numpy())
            np_labels_list.append(y.detach().cpu().numpy())
            np_preds_list.append(pred.detach().cpu().numpy().squeeze(-1))

    test_loss /= count

    all_logits = np.concatenate(np_logits_list, -1)
    all_preds = np.concatenate(np_preds_list, -1)
    all_labels = np.concatenate(np_labels_list, -1)
    weight = None
    eval_dict = eval_overall_performance(all_logits, all_preds, all_labels)
    ...
    if writer is not None:
        writer.add_scalar('eval/Metrics/ACC', eval_dict['acc'], epoch)
        writer.add_scalar('eval/Metrics/AUROC', eval_dict['auroc'], epoch)
        writer.add_scalar('eval/Metrics/T_AP', eval_dict['t_ap'], epoch)
        writer.add_scalar('eval/Metrics/T_F1', eval_dict['t_f1'], epoch)
        writer.add_scalar('eval/Metrics/T_P', eval_dict['t_p'], epoch)
        writer.add_scalar('eval/Metrics/T_R', eval_dict['t_r'], epoch)
        writer.add_scalar('eval/Metrics/F_AP', eval_dict['f_ap'], epoch)
        writer.add_scalar('eval/Metrics/F_F1', eval_dict['f_f1'], epoch)
        writer.add_scalar('eval/Metrics/F_P', eval_dict['f_p'], epoch)
        writer.add_scalar('eval/Metrics/F_R', eval_dict['f_r'], epoch)
        writer.add_scalar('eval/Metrics/ECE', eval_dict['ece'], epoch)

    print(
        '\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.4f}%)\n'.format(
            test_loss, correct, count, 100. * correct / count))
    return test_loss, correct * 1.0 / count, torch.cat(pred_list, dim=0), eval_dict

# %%
def test(model, test_data, args, writer=None, epoch=0):
    model.eval()
    test_loss = 0
    correct = 0
    count = 0
...
def create_datasets(features, shuffle=True):
    ...
    :param features:
    :param choices_num: number of label
...
def create_complex_datasets_with_kbert(features, shuffle=True):
    """
    build dataset with features
    :param features:
    :param choices_num: number of label
...
def train_and_finetune_complex(model, train_data, test_data, args):
    global optimizer
    """
    ...
    #     'pre_weights/albert-xlarge_model.bin', config=config)
    ...
    # fix_idx = len(model_parameters) - 23  # 23 for with gcn and cls
    fix_idx = len(model_parameters) - 23  # 23 for with gcn and cls
    ...
    for idx, (name, i) in enumerate(model.named_parameters()):
        ...
        # in our adv project, the value should be 23
        if idx < fix_idx:
        ...
        for imag_name in imag_name_list:
            if imag_name in name:
                i.requires_grad = True
    print('iterate list done!')
    model.to(args['device'])
    print('mode deploy done!')
    ...
    for epoch in range(args['epochs']):
        ...
        train_loss, train_acc, train_pred = train(model, train_data, optimizer, args)
    ...
    # optimizer = optim.Adam(params=model.parameters(), lr=args['fine_tune_lr'], eps=args['adam_epsilon'])
...
def eval_overall_performance(logit, preds, labels):
    ...
    precision, recall, thresholds = precision_recall_curve([1 - y for y in labels], [- y for y in logits],
...
def train_and_finetune(model, train_data, test_data, args):
    ...
    for epoch in range(args['epochs']):
        ...
        train_loss, train_acc, train_pred = train(model, train_data, optimizer, args, writer=writer, epoch=global_epoch)
        test_loss, test_acc, test_pred = test(model, test_data, args, writer=writer, epoch=global_epoch)
        if test_acc > acc:
            # 在准确率最高的一次 finetune 中保存预测信息
            acc = test_acc
            train_pred_opt = train_pred
    ...
    optimizer = optim.SGD(model.parameters(), lr=args['fine_tune_lr'])
    ...
    for epoch in range(args['fine_tune_epochs']):
        ...
        global_epoch = global_epoch + 1
        train_loss, train_acc, train_pred = train(model, train_data, optimizer, args, writer=writer, epoch=global_epoch)
        test_loss, test_acc, test_pred = test(model, test_data, args, writer=None, epoch=global_epoch)
        # real_test_loss, real_test_acc, real_test_pred = test(model, test_data, args, writer=writer, epoch=global_epoch)
        if test_acc > acc:
            # 在准确率最高的一次 finetune 中保存预测信息
            acc = test_acc
            train_pred_opt = train_pred
        ...
        if writer is not None:
            # eval_overall_performance(None, None)
            writer.add_scalar('Loss/fine-tune train', train_loss, epoch)
            writer.add_scalar('Accuracy/fine-tune train', train_acc, epoch)
            writer.add_scalar('Loss/fine-tune test', test_loss, epoch)
            writer.add_scalar('Accuracy/fine-tune test', test_acc, epoch)
