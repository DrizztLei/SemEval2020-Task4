...
class GCNNet(nn.Module):
    ...
    def forward(self, data):
        ...
        out = self.conv1(x, edge_index, edge_weight) # be4 107,300
        out = gelu(out) # 107 128
        out = F.dropout(out, training=self.training)
        logits = self.conv2(out, edge_index, edge_weight) # be4 107 128 after 107 128
        logits = torch.stack(
            [logits[data.batch == i][data.pos[data.batch == i]].mean(dim=0) for i in range(data.num_graphs)], dim=0)

        out = gelu(logits) # stack -> after 2, 128
        out = self.fc1(out)

        outputs = (out.reshape(-1, data.num_graphs), logits,)

        if data.y is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(outputs[0], data.y.reshape(-1, data.num_graphs).argmax(dim=1))
            outputs = (loss,) + outputs
        # [tuple (a, b, c) -> i[1] j[2] k[2, 128]
        return outputs
