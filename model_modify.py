...
from utils.MyDataset import MyDataLoader, MyDataset, MyParallelDataset, MyLazyLoadDataset
...
def apply_to_device_and_convert(data, device):
    if isinstance(data, list):
        return [apply_to_device_and_convert(item, device) for item in data]
    elif isinstance(data, tuple):
        return tuple(apply_to_device_and_convert(list(data), device))
        ...
        if hasattr(data, 'num_graphs') and isinstance(data.ptr, torch.Tensor):
            data.ptr = data.ptr.to(device)
        return data
    else:
        return data

def parallel_train(model, train_data, optimizer, args, accelerate):
    model.train()
    ...
    if not args['with_mult_write']:
        # logging.basicConfig(level=logging.INFO)
        # log = accelerate.logging.get_logger(__name__, log_level='INFO')
        model, optimizer, train_dl = accelerator.prepare(model, optimizer, train_data)
    else:
        model, optimizer, train_dl = accelerator.prepare(model, optimizer, train_data)
    pbar = tqdm(train_dl)
    for step, (x, y) in enumerate(pbar):
        ...
        x = apply_to_device_and_convert(x, device)
        optimizer.zero_grad()
        if args['solo']:
            ...
            if not is_gnn:
                ...
                # for SOTA model
                # new_x = apply_to_device_and_convert(x, device=args['device'])
        ...
        pred_list.append(output[1].softmax(dim=1).cpu().detach().numpy())
    ...
    pbar.close()
    return train_loss / count, correct * 1.0 / count, np.concatenate(pred_list, axis=0)
...
def log_error(model, test_data, args):
    model.eval()
    test_loss = 0
    correct = 0
    count = 0
        ...
        for step, (x, y) in enumerate(test_data):
            ...
            for input in x:
                ...
                for index in range(input_length):
                    input_sentence = " ".join(input[index][0][0][0].replace('Ġ', ''))
                    tmp_list.append(input_sentence)
            error_index = torch.where(pred.ne(y.view_as(pred)))[0]
            print(len(pred))
            print(len(y))
            print(error_index)
            print(len(tmp_list))
            error_list.append([tmp_list[index] for index in error_index])
            correct += pred.eq(y.view_as(pred)).sum().item()
            count += len(y)

    return error_list

def parallel_test(model, test_data, args, accelerate):
    model.eval()
    test_loss = 0
    correct = 0
    count = 0
    ...
    with torch.no_grad():
        for step, (x, y) in tqdm(enumerate(test_data), total=len(test_data)):
            # x, y = x.to(args['device']), y.to(args['device'])
            x = apply_to_device_and_convert(x, device)
            y = y.to(device, non_blocking=True) #, non_blocking=True)
            # for SOTA model
            output = model(x, labels=y)
            loss = accelerator.gather(output[0]).mean()
            pred = accelerator.gather(output[1]).softmax(dim=1).argmax(dim=1, keepdim=True)
            # loss = output[0].mean()
            test_loss += loss.detach().item()
            # pred = output[1].softmax(dim=1).argmax(dim=1, keepdim=True)
            pred_list.append(output[1].softmax(dim=1))
            correct += pred.eq(y.view_as(pred)).sum().detach().item()
            count += len(y)
            # accelerator.backward(loss)
            del x, y, output
            torch.cuda.empty_cache()
...
def create_lazy_load_datasets_with_kbert(features, shuffle=True):
    return MyLazyLoadDataset(features)

def create_parallel_datasets_with_kbert(features, shuffle=True):
    """
    使用 features 构建 dataset
    :param features:
    :param choices_num: 选项(label) 个数
...
def continue_ft(model, train_data, test_data, args, v3_epoch=4):
    ...
    # 整体进行 fine-tune
    print('start fine-tune v3...')
    for epoch in range(v3_epoch):
        print('Epoch {}/{}'.format(epoch + 1, v3_epoch))
        # np.random.shuffle(train_data)  # 每个 ep 打乱 train data 顺序
        # 每一次的训练从之前最优时候开始
        # load_checkpoint(model=model, args=args, test_acc=acc)

        train_loss, train_acc, train_pred = train_data(model, train_data, optimizer, args)
        test_loss, test_acc, test_pred = test(model, test_data, args)
        if acc is not None and test_acc > acc:
...
def train_and_finetune(model, train_data, test_data, args):
    ...
    no_split_list = [
        ...
        "LayerNormalization","InstanceNorm1d","GraphConvolutionMPS","GATConv",
        "ReLU", "Parameter", "nn.Parameter"]

    max_memory = get_balanced_memory(
        model,
        max_memory=None,
        no_split_module_classes=no_split_list,
        dtype='float16',
        low_zero=False,
    )
    ...
    if torch.cuda.device_count() > 1:
        ...
        print("deploy model to : ", torch.cuda.device_count(), " GPUs")
        print(args['device'])
    """
    ...
    for epoch in range(args['epochs']):
        ...
        train_loss, train_acc, train_pred = parallel_train(model, train_data, optimizer, args, accelerate)
        test_loss, test_acc, test_pred = parallel_test(model, test_data, args, accelerate)
        if test_acc > acc:
            # 在准确率最高的一次 finetune 中保存预测信息
            acc = test_acc
            train_pred_opt = train_pred
...
def train_and_finetune_and_test(model, train_data, dev_data, test_data, args):
    global optimizer
    """
    因为涉及到固定网络部分层权重，目前没有手动设置而是采用在 model 初始化的时候设置，而在交叉验证中因为要多次创建 model，所以暂时将 model 的初始化放在这里
    """
    ...
    for epoch in range(args['epochs']):
        ...
        train_loss, train_acc, train_pred = train(model, train_data, optimizer,
                                                  args)
        test_loss, test_acc, test_pred = test(model, dev_data, args)
        real_test_loss, real_test_acc, real_test_pred = test(model, test_data, args)
        if test_acc > acc:
            # 在准确率最高的一次 finetune 中保存预测信息
            acc = test_acc
            train_pred_opt = train_pred
        ...
        if writer is not None:
            ...
            writer.add_scalar('Loss/test', test_loss, epoch)
            writer.add_scalar('Accuracy/test', test_acc, epoch)
            writer.add_scalar('Loss/real_test', real_test_loss, epoch)
            writer.add_scalar('Accuracy/real_test', real_test_acc, epoch)
